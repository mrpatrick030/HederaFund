...
export default function LendSection() {
  ...
  const showMyCreatedLoans = () => {
    ...
    const fundLoanFromLoanList = async (initialID, initialLoanAmount) => {  
        if(isConnected){
         ...
         try {
            const lendtoken = await lendBorrowContractWriteSettings.fundLoan(initialID, {value:initialLoanAmount})
        } catch (error) {
          console.log(error)
          setLoading(false)
         }
 ...
 {searchData.length > 0 ? 
  (<div className="my-[1cm] bg-[#000] p-[0.5cm] rounded-xl" style={{boxShadow:"2px 2px 2px 2px #222"}}>
    ...
    <div className="m-[0.2cm]" style={{display:"inline-block"}}><span className="bg-[#209] rounded-md px-[0.3cm] py-[0.1cm] text-[80%]" style={{border:"2px solid #333"}}>Amount to borrow</span> {parseFloat(data.amount.toString() * 10**-18).toFixed(10)} HBAR</div>
    ...
    <div className="m-[0.2cm]" style={{display:"inline-block"}}><span className="bg-[#209] rounded-md px-[0.3cm] py-[0.1cm] text-[80%]" style={{border:"2px solid #333"}}>Repayment amount</span> {parseFloat(data.repaymentAmount.toString() * 10**-18).toFixed(10)} HBAR</div>
